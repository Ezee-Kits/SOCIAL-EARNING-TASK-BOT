...
async def YT_Task_Bot(page,task,task_url):
    ...
    await xpath_click_center(page,"//a[contains(text(), 'View Job')]")
    ...
    if new_pages:
        ...
        if 'YOUTUBE/Channel Subscribe---Video like---Video Comment'.lower() in task.lower():
            ...
            try:
                ...
                is_liked = await new_tab.evaluate(
                    f'document.querySelector(\'{like_button_selector}\').getAttribute("aria-pressed")')
                if is_liked == 'false':
                    await new_tab.click(like_button_selector)
                    print("Video liked ✅")
                else:
                ...
                # await new_tab.waitForSelector('#placeholder-area #simplebox-placeholder', {'visible': True})
            ...
            await asyncio.sleep(3)  # wait a bit to see the action
