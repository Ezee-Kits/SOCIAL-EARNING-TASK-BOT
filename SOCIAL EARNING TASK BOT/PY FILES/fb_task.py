...
async def FB_Task_Bot(page,task,task_url):
    ...
    if new_pages:
        ...
        if 'facebook' in new_tab.url.lower():
            print("✅ New tab loaded with Facebook URL.")
        else:
            print("❌ New tab does not have a Facebook URL.")
            return False
        ...
        elif 'FACEBOOK/Post Like'.lower() in task.lower():
            await asyncio.sleep(5)
            try:
        ...
        elif 'FACEBOOK/Group Join'.lower() in task.lower():
            await asyncio.sleep(1)
            try:
                join_group_script = """
                () => {
                    const joinBtn = document.querySelector('[role="button"][aria-label="Join Group"]');

                    if (joinBtn) {
                        joinBtn.scrollIntoView({ behavior: "smooth", block: "center" });
                        joinBtn.click();
                        return "✅ 'Join Group' button clicked successfully!";
                    } else {
                        return "❌ 'Join Group' button not found on the page.";
                    }
                }
                """
                ...
                await new_tab.waitForSelector('div[role="button"]',timeout=10000)
                done_task = 1
                result = await new_tab.evaluate(join_group_script)
                print(result)
