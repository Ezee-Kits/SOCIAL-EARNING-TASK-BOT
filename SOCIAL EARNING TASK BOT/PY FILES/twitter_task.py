...
async def X_Task_Bot(page,task,task_url):
    ...
    if new_pages:
        ...
        if 'x.com' in new_tab.url.lower():
            print("✅ New tab loaded with Twitter URL.")
        else:
            print("❌ New tab does not have a Twitter URL.")
            return False
        ...
        if 'X/Page Follow'.lower() in task.lower():
            try:
                ...
                done_task = 1
                await css_scroll_center(new_tab,'button[role="button"]')
