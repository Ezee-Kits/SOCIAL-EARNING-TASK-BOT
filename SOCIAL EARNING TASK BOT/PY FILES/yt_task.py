...
async def YT_Task_Bot(page,task,task_url):
    ...
    if new_pages:
        ...
        if 'youtube' in new_tab.url.lower():
            print("✅ New tab loaded with YouTube URL.")
        else:
            print("❌ New tab does not have a YouTube URL.")
            return False
        ...
        if 'YOUTUBE/Channel Subscribe---Video like---Video Comment'.lower() in task.lower():
            ...
            try:
                ...
                await new_tab.waitForSelector(subscribe_button_selector, timeout=20000)
                ...
                #=================== VIDEO COMMENT =======================
                await new_tab.waitForSelector('video', timeout=10000)
                for i in range(8):  # scroll 10 times
        ...
        #     await asyncio.sleep(10)  # Wait for 5 minutes (20 seconds)
            ...
            try:
                subscribe_button_selector = '#subscribe-button yt-button-shape button'
                await new_tab.waitForSelector(subscribe_button_selector, timeout=15000)
                clicked = await new_tab.evaluate(f'''
                    (function() {{
                        const subscribeBtn = document.querySelector("{subscribe_button_selector}");
                        if (subscribeBtn) {{
        ...
        elif 'YOUTUBE/Video Likes'.lower() in task.lower():
            try:
